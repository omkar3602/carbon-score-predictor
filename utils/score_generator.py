...
def get_carbon_score(oxygen_val, home_appliance_co2_val, vehicle_co2_val, waste_management_co2_val, user):
        ...
        if net_carbon_emission < 0:
                send_mail(user.email, f"Check your Carbon score.", f"Hey {user.fullname}. Congratulations!! Your carbon score is net-negative ({round(net_carbon_emission, 2)}). Check it out here {os.getenv('WEB_URL')}#carbon_score")
        else:
                send_mail(user.email, f"Check your Carbon score.", f"Hey {user.fullname}. You need to work on reducing your carbon footprint. Your carbon score is net-positive ({round(net_carbon_emission, 2)}). Check it out here {os.getenv('WEB_URL')}#carbon_score")
